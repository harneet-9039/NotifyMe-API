...
static facultyUser(req,res)
{
      ...
      var array1=JSON.parse(req.body.year);
               ...
               var sql=' (select n.Notice_id,d.Dept_name as department, c.Course_branch as course, s.name ,CAST(s.contact AS char) as contact, s.Reg_id, n.validity,n.priority,n.title,n.description,n.date_time,s.email_id,s.isCoordinator ,s.eventName,sc.scope,GROUP_CONCAT(distinct a.File_path) Attachments,GROUP_CONCAT(distinct i.Image_path) Images from  student_registration s inner join department d on s.dept_id=d.Dept_id inner join courses c on d.Dept_id=c.Dept_id inner join notices n on n.S_sender_id=s.Reg_id left join attachments a on n.Notice_id=a.NoticeID left join images i on n.Notice_id=i.NoticeID inner join scope sc on n.Notice_id=sc.NoticeID  where (sc.scope = 1 or (sc.scope=2 and sc.Dept_id ='+connection.escape(dept_id)+') or (sc.scope=3 and  sc.Course_id in (';
   ...
   var i;
       for(i=0;i<array.length;i++)
       {
         sql+=' '+connection.escape(array[i])+' and sc.year= '+connection.escape(array1[i])+') or (scope=4 and sc.Course_id='
               ...
               sql+=' -12)) group by Notice_id) union (select n.Notice_id,d.Dept_name as department, "" as course, f.Name as name,CAST(f.contact AS char) as contact, f.Faculty_id as Reg_id, n.validity,n.priority,n.title,n.description,n.date_time,f.email_id,f.designation as isCoordinator ,"" as eventName,sc.scope,GROUP_CONCAT(distinct a.File_path) Attachments,GROUP_CONCAT(distinct i.Image_path) Images from  faculty_registration f inner join department d on f.dept_id=d.Dept_id inner join courses c on d.Dept_id=c.Dept_id inner join notices n on n.F_sender_id=f.Faculty_id left join attachments a on n.Notice_id=a.NoticeID left join images i on n.Notice_id=i.NoticeID inner join scope sc on n.Notice_id=sc.NoticeID  where (sc.scope = 1 or (sc.scope=2 and sc.Dept_id ='+connection.escape(dept_id)+') or (sc.scope=3 and  sc.Course_id in (';
